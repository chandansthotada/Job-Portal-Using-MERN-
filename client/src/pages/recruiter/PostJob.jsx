...
const PostJob = () => {
  ...
  const [form, setForm] = useState({
    ...
    isRemote: false, jobType: '', experience: '',
    salaryMin: '', salaryMax: '', isSalaryHidden: false,
    vacancy: 1, deadline: '',
  ...
  // Hiring process
  const addStep = () =>
    setForm({ ...form, hiringProcess: [...form.hiringProcess, { step: form.hiringProcess.length + 1, title: '', description: '' }] });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await API.post('/jobs', {
        ...
        isSalaryHidden:   form.isSalaryHidden,
  ...
  return (
    <div style={styles.wrap}>
      <div style={styles.card}>
        ...
        <p style={styles.subtitle}>Fill in the details to attract the best candidates</p>
            ...
            <div className="tag-wrap">
              {form.skills.map((s) => (
                <span key={s} className="tag-item">
                  {s} <button type="button" onClick={() => removeTag('skills', s)}><FaTimes size={10} /></button>
                </span>
              ))}
            </div>
          </div>
            ...
            <div className="tag-wrap">
              {form.perks.map((p) => (
                <span key={p} className="tag-item">
                  {p} <button type="button" onClick={() => removeTag('perks', p)}><FaTimes size={10} /></button>
                </span>
              ))}
            </div>
          </div>
...
const styles = {
  ...
  title:          { fontSize: '1.8rem', fontWeight: 800, color: '#111827', marginBottom: 4 },
  subtitle:       { color: '#6b7280', marginBottom: 28, fontSize: '0.92rem' },
